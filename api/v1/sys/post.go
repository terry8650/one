...
type PostSearchRes struct {
	...
	common.ListRes
	PostList []*entity.SysPost `json:"data"`
}
...
type PostEditReq struct {
	...
	PostId   int64  `p:"id" v:"required#id必须"`
	PostCode string `p:"code" v:"required#岗位编码不能为空"`
	PostName string `p:"name" v:"required#岗位名称不能为空"`
	PostSort int    `p:"sort" v:"required#岗位排序不能为空"`
	Status   uint   `p:"status" v:"required#状态不能为空"`
	Remark   string `p:"remark"`
}
